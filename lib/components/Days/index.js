            ...
            {this.weeks.map((innerWeek, i) => {
                return (<tr key={i}>
                    {innerWeek.map((days, j) => {
                        if (this.props.isBefore(days) || this.props.isAfter(days)) {
                            return <td className="staleDay" key={j}>{days}</td>;
                        }
                        if (this.props.isBetween(days, this.props.hoverDate, true)) {
                        ...
                        if (days === startDate && this.props.month === startMonth) {
                            return <td className="startSelected" key={j} onClick={this.selectStartEnd.bind(this, days)}>{days}</td>;
                        }
                        if (days === endDate && this.props.month === endMonth) {
                            return <td className="endSelected" key={j} onClick={this.selectStartEnd.bind(this, days)}>{days}</td>;
                        }
                        if (startDate && endDate && days != null) {
                            if (this.props.isBetween(days, this.props.endDate)) {
