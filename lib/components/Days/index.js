            ...
            {this.weeks.map((innerWeek, i) => {
                return (<tr key={i}>
                    {innerWeek.map((days, j) => {
                        ...
                        if (days >= startDate && days <= endDate && days != null && this.props.month === startMonth && this.props.month === endMonth) {
                            return <td className="midSelected" key={j} onClick={this.selectStartEnd.bind(this, days)}>{days}</td>;
                        }
